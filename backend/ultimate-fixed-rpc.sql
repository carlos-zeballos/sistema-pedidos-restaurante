...
-- =========================================================
-- RPC ULTIMATE CORREGIDO - SIN AMBIGÜEDADES Y CASING CORRECTO
-- =========================================================
-- Versión ultimate que elimina completamente las referencias ambiguas
-- y maneja correctamente el casing de orderNumber
-- =========================================================

-- Primero eliminar cualquier versión anterior
...
BEGIN
  ...
  -- Verificar espacio (usando alias explícito)
  SELECT 1 INTO v_dummy FROM "Space" AS s WHERE s.id = p_space_id AND s."isActive" IS TRUE;
  IF NOT FOUND THEN
    RAISE EXCEPTION 'El espacio % no existe o no está activo', p_space_id;
  END IF;
  
  -- Verificar usuario (usando alias explícito)
  SELECT 1 INTO v_dummy FROM "User" AS u WHERE u.id = p_created_by AND u.isactive IS TRUE;
  IF NOT FOUND THEN
  ...
  LOOP
    ...
    IF (v_item ? 'productId') THEN
      ...
      -- Producto habilitado + disponible (usando alias explícito)
      PERFORM 1 FROM "Product" AS p
    ...
    ELSIF (v_item ? 'comboId') THEN
      ...
      -- Combo habilitado + disponible (usando alias explícito)
      PERFORM 1 FROM "Combo" AS c
    ...
    ELSE
      IF v_product_id IS NOT NULL THEN
        SELECT p.price::numeric INTO v_unit_price FROM "Product" AS p WHERE p.id = v_product_id;
      ELSE
        SELECT c."basePrice"::numeric INTO v_unit_price FROM "Combo" AS c WHERE c.id = v_combo_id;
      END IF;
    END IF;
  ...
  -- ===== 4) Insertar cabecera (sin RETURNING para evitar problemas de casing) =====
  ...
  IF v_order_id IS NULL THEN
    -- Si no hay secuencia, usar el último ID insertado
  ...
  LOOP
    ...
    IF v_name IS NULL THEN
      IF v_product_id IS NOT NULL THEN
        SELECT p.name INTO v_name FROM "Product" AS p WHERE p.id = v_product_id;
      ELSE
        SELECT c.name INTO v_name FROM "Combo" AS c WHERE c.id = v_combo_id;
      END IF;
    END IF;
    
    INSERT INTO "OrderItem" AS oi(
    ...
    IF (v_item ? 'components') AND jsonb_typeof(v_item->'components') = 'array' THEN
      ...
      LOOP
        ...
        IF v_comp_id IS NOT NULL THEN
          ...
          ELSE
            SELECT cc.name INTO v_comp_name FROM "ComboComponent" AS cc WHERE cc.id = v_comp_id;
          END IF;
          
          IF (v_comp ? 'type') THEN
            v_comp_type := v_comp->>'type';
          ELSE
            SELECT cc.type INTO v_comp_type FROM "ComboComponent" AS cc WHERE cc.id = v_comp_id;
          END IF;
          
          IF (v_comp ? 'price') THEN
            v_comp_price := (v_comp->>'price')::numeric;
          ELSE
            SELECT COALESCE(cc.price,0)::numeric INTO v_comp_price FROM "ComboComponent" AS cc WHERE cc.id = v_comp_id;
          END IF;
        ELSE
        ...
        INSERT INTO "OrderItemComponent" AS oic(
